 ...
 * Il va prendre a partir de la matrice couleur dans quelle equipe est le roi puis va tester tout les deplacements possibles
 ...
 */
QVector<QPoint> deplacements::deplacementRoi(const char idchar[8][8],const int couleur[8][8], const QPoint& cas)
{
                ...
                for(int y = -1; y < 2; y++) // pour chaque x on fait les 3 valeurs de y possible
                {
                    ...
                    if((cas.y()+y) < 8 && (cas.y()+y) >= 0 && (cas.y() != (cas.y()+y) || cas.x() != (cas.x()+x)))
                    {
                        if(couleur[cas.x()+x][cas.y()+y] != couleur[cas.x()][cas.y()] )
                        {
                            if(x != 0 || y != 0)
                            {
                              if(Plateau::est_en_echec(QPoint(cas.x(),getCoordonne.getY()),NULL,idJoueur,couleur,idchar) == false)
                              {
...
/**
 * \brief deplacements::checkPetitRoque()
 ...
 */
bool deplacements::checkPetitRoque(int joueur,const int couleur[8][8],const int matriceDeplacement[8][8],const char idPiece[8][8])
{
...
/**
 * \brief deplacements::checkGrandRoque()
 ...
 * \return True si on peut faire le grand rogue, sinon False
 */
bool deplacements::checkGrandRoque(int joueur,const int couleur[8][8],const int matriceDeplacement[8][8],const char idPiece[8][8])
{
    if(joueur == 1) // joueur noir
    {
        // si le roi et la tour n'ont pas bouge
 ...
 * \return Un tableau de coordonnees
 */
QVector<QPoint> deplacements::deplacementFou(const int couleur[8][8], const QPoint& cas){
    ...
    QVector<QPoint> resultat;
        ...
        while (x > 0 && y > 0)
        {
            ...
            if(couleur[x][y] != 0) // on a rencontre un obstacle ami ou ennemis
            {
        ...
        while (x > 0 && y < 7)
        {
            ...
            if(couleur[x][y] != 0) // on a rencontre un obstacle ami ou ennemi
            {
                x = 0; // on stoppe la boucle
            }
        }
 ...
 * La fonction va prendre a partir de la matrice couleur dans quelle equipe est le Cavalier puis va tester tous les deplacements possibles
 ...
 * \return Un tableau de coordonnees
 */
QVector<QPoint> deplacements::deplacementCavalier(const int couleur[8][8], const QPoint& cas){
    QVector<QPoint> resultat;
        /////////// HAUT
        if(cas.y()>= 1){
 ...
 * La fonction va prendre a partir de la matrice couleur dans quelle equipe est la Tour puis va tester tout les deplacements possibles
 ...
 * \return Un tableau de coordonnees
 */
QVector<QPoint> deplacements::deplacementTour(const int couleur[8][8], const QPoint& cas){
    QVector<QPoint> resultat;
        int x = cas.x();
        while(x > 0)
        {
 ...
 * La fonction va prendre a partir de la matrice couleur dans quelle equipe est le pion puis va tester tout les deplacements possibles
 ...
 * \return Un tableau de coordonnees
 */
QVector<QPoint> deplacements::attaquePion(const int couleur[8][8],const QPoint& cas)
{
    QVector<QPoint> resultat;
    bool noir = true;
 ...
 * \return Un tableau de coordonnees
 */
QVector<QPoint> deplacements::deplacementPion(int idJoueur,const int couleur[8][8],const char idchar[8][8],const QPoint& cas,QVector<QPoint> *coup_precedent)
{
    QVector<QPoint> resultat;
        bool noir = true;
        ...
        if(noir == true &&   cas.x() == 1 && couleur[cas.x()+2][cas.y()] == -1 && couleur[cas.x()+1][cas.y()] == -1)
        {
            resultat.append(QPoint(cas.x()+2,cas.y()));
        }
        else if(noir == false && cas.x() == 6 && couleur[cas.x()-2][cas.y()] == -1 && couleur[cas.x()-1][cas.y()] == -1)
        {
            resultat.append(QPoint(cas.x()-2,cas.y() ));
        }
        ...
        if(noir == true)
        {
            ...
            if(idchar[dest_precedent.x()][dest_precedent.y()] == Pion  && origin_precedent.x() == 6)
            {
                // si le pion fait pas parti de mon couleur
                if(couleur[dest_precedent.x()][dest_precedent.y()] != couleur[cas.x()][cas.y()])
                {
        ...
        else
        {
            ...
            if(idchar[dest_precedent.x()][dest_precedent.y()] == 'P'  && origin_precedent.x() == 1)
            {
                // si le pion fait pas parti de mon couleur
                if(couleur[dest_precedent.x()][dest_precedent.y()] != couleur[cas.x()][cas.y()])
                {
 ...
 */
bool deplacements::inCheck(const char idPiece[8][8],const int couleur[8][8],int couleur, const QPoint& coordcase, const QPoint* coordpion)
{   // on teste pour toutes les chars qui ne sont pas dans mon couleur si il y en a une qui mene a ma char , si oui elle est en echec
    ...
    for(int x = 0; x < 8; x++)
    {
        for(int y = 0; y < 8; y++)
        {
            if(couleur[x][y] != couleur )
            {
                switch(idPiece[x][y])
                {
                    case 'P':
                        destination = deplacements::attaquePion(couleur,QPoint(x,y));
                    ...
                    case 'R':
                        if(coordpion != NULL)
                        {
                            ...
                            {
                                destination = deplacements::deplacementRoi(idPiece,couleur,QPoint(x,y));
                            }
                            else
                            {
                                //on ne peut simplement ignorer l'autre roi, il faut calculer si on peut etre sur la char
                            }
                    ...
                    break;
                    case Reine:
                        destination = deplacements::deplacementReine(couleur,QPoint(x,y));
                        for(int u = 0; u < destination.count(); u++)
                        {
                            if(destination.at(u).x() == coordcase.x() && destination.at(u).y() == coordcase.y() )
                            {
                    ...
                    break;
                    case 'F':
                        destination = deplacements::deplacementFou(couleur,QPoint(x,y));
                        for(int u = 0; u < destination.count(); u++)
                        {
                            if(destination.at(u).x() == coordcase.x() && destination.at(u).y() == coordcase.y() )
                            {
                    ...
                    break;
                    case 'T':
                        destination =  deplacements::deplacementTour(couleur,QPoint(x,y));
                        for(int u = 0; u < destination.count(); u++)
                        {
                            if(destination.at(u).x() == coordcase.x() && destination.at(u).y() == coordcase.y() )
                            {
                    ...
                    break;
                    case 'C':
                        destination = deplacements::deplacementCavalier(couleur,QPoint(x,y));
                        for(int u = 0; u < destination.count(); u++)
                        {
                            if(destination.at(u).x() == coordcase.x() && destination.at(u).y() == coordcase.y() )
                            {
